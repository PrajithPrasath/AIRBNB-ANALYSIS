...
tab1, tab2 = st.tabs(["HOME", "ANALYZE"])
...
with tab1:
    ...
    st.sidebar.subheader(":white[Project Process Overview]")
    st.sidebar.markdown("- Data Extraction")
    st.sidebar.markdown("- Data Cleaning and Transformation")
    st.sidebar.markdown("- Visualization Development with Streamlit")
    st.sidebar.markdown("- Seasonal Availability Analysis")
    st.sidebar.markdown("- Geospatial Insights")
    st.sidebar.markdown("- Interactive Visualization Features")
    st.sidebar.markdown("- Dashboard Design and Implementation")

    # Define information
    Project_Overview = "Analyzing Airbnb data entails detailed cleaning and preprocessing, paving the way for captivating geospatial visualizations and dynamic plots. These unveil the intricate dynamics of pricing, trends in availability, and unique insights tailored to specific locations."

    Airbnb = "Airbnb is a global online marketplace that connects travelers with hosts offering lodging and experiences. The company earns revenue through commissions from each booking made through its platform, facilitating a wide range of accommodation options worldwide."

    Streamlit = "Streamlit is a simple and powerful tool in Python that helps you quickly create interactive web applications for your data projects. It's designed to make it easy for anyone, especially data scientists and developers, to turn their data into visually appealing apps without needing extensive coding knowledge."
    
    PowerBI = "Power BI harnesses data to illuminate insights with stunning visualizations and robust analytics, empowering informed decision-making like never before, making it a cornerstone of modern business intelligence."

    Technology_Stack = "Python, VS Code, GitHub, Streamlit, PowerBI"
...
#To allow the user to filter by country,property type, room type and view related plots
    col1,col2,col3=st.columns(3)
    with col1:
       country =st.selectbox('Select a Country',sorted(df.country.unique()))
    with col2:  
       property = st.selectbox('Select Property_type',sorted(df.property_type.unique()))
    with col3:    
       room = st.selectbox('Select Room_type',sorted(df.room_type.unique()))                      
            
    #To view avarage listing of  top 10 hosts
    df_1=df[(df['country']==country)&(df["property_type"]==property) &  (df["room_type"]==room)]
    ...
    df_bed_avai=df[(df['country']==country)&(df["property_type"]==property) &  (df["room_type"]==room)]
    ...
    if select==":red[Price Analysis]":
        ...
        fig_1= px.bar(df3.head(20), x= "name",  y= "Price" ,color= "Price",color_continuous_scale= "rainbow",
                      range_color=(0,df3.head(20)["Price"].max()),title= "MINIMUM_NIGHTS MAXIMUM_NIGHTS AND ACCOMMODATES",
    ...
    if select == ":red[Availability Analysis]":
        ...
        df_sunb= px.sunburst(df, path=["room_type","bed_type","host_identity_verified"],values="availability",
                             width=600,height=500,title="Availability of property with host_identity_verification",color_discrete_sequence=px.colors.sequential.Aggrnyl_r)
